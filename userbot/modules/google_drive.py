...
async def generate_credentials(gdrive):
    ...
    if G_DRIVE_DATA is not None:
        try:
            ...
            await gdrive.edit("`[AUTHENTICATE - ERROR]`\n\n"
                              "`Status` : **BAD**\n"
    ...
    try:
        ...
        try:
            ...
            if "/view" in uri:
                ...
                try:
                    file_Id = uri.split("uc?export=download&confirm=")[1].split(
                        "id=")[1]
                except IndexError:
...
async def change_permission(service, Id):
    ...
    try:
        ...
        if f'"File not found: {Id}."' in str(
                e
        ) or ('"Sharing folders that are inside a shared drive is not supported."'
              in str(e)):
            return
        else:
            raise e
...
async def task_directory(gdrive, service, folder_path):
    ...
    for f in lists:
        ...
        if isdir(current_f_name):
            ...
            parent_Id = folder.get("id")
            root_parent_Id = await task_directory(gdrive, service,
                                                  current_f_name)
        else:
...
@register(
    pattern=r"^\.gdlist(?: |$)(-l \d+)?(?: |$)?(.*)?(?: |$)", outgoing=True)
async def lists(gdrive):
    ...
    if len(message) > 4096:
        ...
        await gdrive.edit("**Google Drive Query**:\n"
                          f"`{query}`\n\n**Results**\n\n{message}")
    return
    ...
    for name_or_id in f_name:
        ...
        result = (
            service.files().list(
                q=f'name="{name_or_id}"',
                spaces="drive",
                fields=("nextPageToken, files(parents, name, id, size, "
        ...
        if exe == "mkdir":
            ...
            try:
                service.files().delete(
                    fileId=f_id, supportsAllDrives=True).execute()
            except HttpError as e:
...
async def google_drive(gdrive):
    ...
    file_path = None
    uri = None
    if not (value or gdrive.reply_to_msg_id):
        return None
    ...
    if isfile(value):
        ...
        try:
            ...
            await gdrive.edit("`[FOLDER - UPLOAD]`\n\n"
                              f"`{folder_name}`\n"
                              "`Status` : **BAD**\n"
                              f"`Reason` : {str(e)}")
            await reset_parentId()
        ...
        if re.findall(r"\bhttps?://drive\.google\.com\S+", value):
            ...
            if reply:
                ...
                return None
        if not (uri or gdrive.reply_to_msg_id):
            await gdrive.edit(
                "`[VALUE - ERROR]`\n\n"
                "`Status` : **BAD**\n"
...
async def set_upload_folder(gdrive):
    ...
    try:
        ...
        if True in [c1 or c2]:
            parent_Id = inp
            await gdrive.edit("`[PARENT - FOLDER]`\n\n"
